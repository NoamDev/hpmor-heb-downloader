...
class DownloadThrd (Thread):
   def __init__(self, file_id, file_name, destination,flags):
   ...
   def run(self):
      try:
          single_downloader(self.file_id, self.file_name, self.destination)
          self.flags.append((self.file_name,True))
      except:
          self.flags.append((self.file_name,False))
...
def single_downloader(file_id, file_name, destination):
    ...
    default_url = 'https://docs.google.com/document/d/FILE_ID/export?format=pdf'
...
def complete_downloader_pathless(chapter_dict):
    ...
    for key, value in chapter_dict.items():
        i=i+1
        t=DownloadThrd(value, key, new_path,flags)
        t.start()
        threads.append(t)
        files.append(new_path + os.sep + key + file_format)
    for t in threads:
        t.join()
    for key,flag in collections.OrderedDict(flags).items():
        if not flag:
            single_downloader(chapter_dict[key],key,new_path)
...
def complete_unifier_pathless(chapter_dict):
    ...
    def merge(doc_list):
        ...
        for pdf in doc_list:
            ...
            print('chapter ' + pdf[-7:-4] + ' merged')
        
        with open(unified_file_name+file_format, 'wb') as fh:
            pdf_writer.write(fh)
        print('\nall files merged successfully')
    merge(complete_downloader_pathless(chapter_dict))
